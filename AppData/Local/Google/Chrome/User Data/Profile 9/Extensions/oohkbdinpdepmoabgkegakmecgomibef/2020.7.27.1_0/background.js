...
function OnBrowserMessage(request, sender, sendResponse)
{
	...
	else if(request.type == "TwoFASSOLoadForm" && WebSSOAuth)
	{
		var pForm = request.detail;
...
{
	/* Note the following code will be supported in
	   Chrome 85 version. When this is avaialble this
	   will allow us to check if the native host is
	   installed. */
	/*   
	navigator.getInstalledRelatedApps().then(relatedApps => {
	  for (let app of relatedApps) {
		trace("background -> app.platform = " + app.platform);
		trace("background -> app.url = " + app.url);
		trace("background -> app.id = " + app.id);		
	  }
	});
	*/ 
	...
	{	
		var hostname = "com.rapididentity.chromechannel";	
		trace("background -> Connecting to native host " + hostname);
		port = browser.runtime.connectNative(hostname);
		trace("background -> connectNative port = " + port);
		if(port != undefined)
		{	
			port.onMessage.addListener(OnNativeMessage);
			SendNativeMessage("AuthRequest", true);
		}		
		trace("background -> Connection attempt done");
	}
}
